...
const PHONE_DISPLAY = "0530 831 10 56";
const PHONE_TEL = "+905308311056";
const WA = "905308311056";
const DOMAIN = "https://arisoymetalhurda.com.tr";
const BRAND = "Arısoy Metal Geri Dönüşüm Hurdacılık";
const FAVICON = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%230d3b3e' stroke-width='2'%3E%3Cpath d='M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z' fill='%23d99a2b'/%3E%3Cpath d='m9 12 2 2 4-4'/%3E%3C/svg%3E";
...
<meta property="og:image" content="${DOMAIN}/img/mersin-tesis.avif">
...
<header class="header">
  <div class="container nav">
    <a href="index.html" class="logo" aria-label="Arısoy Metal ana sayfa">
      <span style="display:flex;align-items:center;gap:10px;font-weight:800;font-size:1.3rem;color:#0d3b3e">
        <svg viewBox="0 0 24 24" width="34" height="34" fill="none" stroke="#d99a2b" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>
...
/* ===================== HİZMETLERİMİZ ===================== */
const servCards = [
  ["demir-celik-hurda.html", "mersin-demir-celik-hurdasi.avif", "Demir-Çelik Hurda", "İnşaat demiri, sac, profil, makine ve döküm hurdası; güncel piyasa fiyatından, yerinde tartım ile alınır.", '<rect x="3" y="9" width="18" height="7" rx="1"/><path d="M3 9l3-5h12l3 5"/>'],
  ["bakir-hurda.html", "mersin-bakir-hurdasi.avif", "Bakır Hurda", "Temiz bakır, kablo bakırı, radyatör ve sarı pirinç; türüne göre en yüksek güncel fiyattan değerlendirilir.", '<circle cx="12" cy="12" r="9"/><path d="M9 9h6v6H9z"/>'],
  ["aluminyum-hurda.html", "mersin-aluminyum-hurdasi.avif", "Alüminyum Hurda", "Doğrama profili, jant, döküm ve levha alüminyum; her ölçekte yerinde tartım ve anında ödeme ile alınır.", '<rect x="4" y="4" width="16" height="16" rx="2"/><path d="M4 12h16"/>'],
  ["kablo-hurda.html", "mersin-kablo-hurdasi.avif", "Kablo Hurda", "Elektrik, enerji ve data kablolarınız içerdiği bakır/alüminyum oranına göre değerlendirilir.", '<path d="M4 4v6a4 4 0 004 4h8a4 4 0 014 4v6"/><circle cx="4" cy="4" r="2"/><circle cx="20" cy="20" r="2"/>'],
  ["beyaz-esya-hurdasi.html", "mersin-beyaz-esya.avif", "Beyaz Eşya Hurdası", "Buzdolabı, çamaşır makinesi, bulaşık makinesi ve fırınınızı adresinizden alıyor, kapıda ödemesini yapıyoruz.", '<rect x="5" y="2" width="14" height="20" rx="1.5"/><path d="M5 9h14"/><path d="M8 5v2M8 12v3"/>'],
  ["klima-hurdasi.html", "mersin-is-makinesi.avif", "Klima Hurdası", "Split, salon tipi ve VRF klimalarınızın iç/dış ünitesini söküp, bakır/alüminyum radyatör değerine göre alıyoruz.", '<rect x="2" y="6" width="20" height="8" rx="2"/><path d="M6 18v2M12 18v2M18 18v2"/>'],
  ["aku-hurdasi.html", "mersin-aku-hurdasi.avif", "Akü Hurdası", "Araç, iş makinesi ve UPS akülerinizi kurşun içeriğine göre değerlendirip yerinde nakit ödüyoruz.", '<rect x="3" y="8" width="16" height="10" rx="1"/><path d="M19 11h2v4h-2zM7 8V6h4v2"/>'],
];
...
const servGrid = servCards
  .map(
    ([href, img, title, desc, icon]) => `      <article class="serv-card reveal">
        <div class="serv-img">
          <img src="img/${img}" alt="Mersin ${title.toLowerCase()}" loading="lazy">
...
writeFileSync(
  ...
  shell({
    slug: "hizmetlerimiz",
    title: "Hizmetlerimiz",
    desc: "Arısoy Metal Geri Dönüşüm Hurdacılık - demir-çelik, bakır, alüminyum, kablo, beyaz eşya, klima ve akü hurdası alım hizmetlerimizin tamamı.",
    breadcrumb: "Hizmetlerimiz",
    heroTitle: "Hizmetlerimiz",
    heroDesc: "Mersin genelinde tüm hurda metal türlerinde profesyonel alım hizmeti veriyoruz. Aşağıdan hurda türünüzü seçip detaylı bilgi alabilirsiniz.",
...
writeFileSync(
  ...
  shell({
    slug: "iletisim",
    title: "İletişim",
    desc: "Arısoy Metal Geri Dönüşüm Hurdacılık iletişim bilgileri: telefon, WhatsApp, adres ve çalışma saatleri.",
    breadcrumb: "İletişim",
    heroTitle: "İletişim",
...
writeFileSync(
  ...
  shell({
    ...
    desc: "Arısoy Metal Geri Dönüşüm Hurdacılık hakkında merak edilenler: fiyatlandırma, ödeme, hizmet bölgeleri ve randevu süreci.",
    breadcrumb: "S.S.S.",
...
function legalPage(slug, title, sections) {
  ...
  writeFileSync(
    ...
    shell({
      slug,
      title,
      desc: `Arısoy Metal Geri Dönüşüm Hurdacılık ${title.toLowerCase()} metni.`,
      breadcrumb: title,
      heroTitle: title,
      heroDesc: "",
...
legalPage("gizlilik-politikasi", "Gizlilik Politikası", [
  ["Genel", ["Arısoy Metal Geri Dönüşüm Hurdacılık olarak, bu web sitesi üzerinden bizimle iletişime geçen ziyaretçilerin kişisel verilerinin güvenliğine önem veriyoruz. Bu metin, hangi bilgilerin toplandığını ve nasıl kullanıldığını açıklar."]],
  ...
  ["İletişim", [`Gizlilik politikamızla ilgili sorularınız için <a href="mailto:info@arisoymetalhurda.com.tr">info@arisoymetalhurda.com.tr</a> adresinden veya ${PHONE_DISPLAY} numaralı telefondan bize ulaşabilirsiniz.`]],
]);
...
legalPage("kullanim-sartlari", "Kullanım Şartları", [
  ...
  ["Hizmet Kapsamı", ["Sitede yer alan bilgiler Arısoy Metal Geri Dönüşüm Hurdacılık'ın sunduğu hurda metal alım hizmetlerini tanıtım amaçlı içerir. Sitede yer alan fiyat aralıkları veya örnek ifadeler bağlayıcı bir teklif niteliği taşımaz; güncel fiyat telefon/WhatsApp üzerinden teyit edilir."]],
  ["Fikri Mülkiyet", ["Sitedeki metin, görsel ve tasarım unsurları Arısoy Metal Geri Dönüşüm Hurdacılık'a aittir; izinsiz kopyalanamaz veya çoğaltılamaz."]],
